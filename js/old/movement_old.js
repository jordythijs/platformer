            ...
            $("#player").promise().done(
                function () {
                    ...
                    var debug = parseInt(
                        $("#player")
                            .css("text-indent")
                            .substring(0, ($("#player").css("text-indent").length - 2))
                    );
                    $("#player").animate(
                        {
                            textIndent: 0
                        }, {
                            duration: 1000, easing: 'linear', progress: function () {
                                debug = parseInt(
                                    $("#player")
                                        .css("text-indent")
                                        .substring(0, ($("#player").css("text-indent").length - 2))
                                ...
                                var y2 = parseInt(
                                    $("#player")
                                        .css("bottom")
                                        .substring(0, ($("#player").css("bottom").length - 2))
                                );
                                var y1 = y2 - 2;
    ...
    if (direction == "up") {
        if (playerIsDucking) {
            ...
            if (foundObject === false) {
              ...
              if (window[foundObject].canGoThrough && !window[foundObject].isDangerousToTouch) {
                return;
              }
                allowedMove = false;
                return false;
            ...
            if (foundObject === false) {
                ...
                maxJumpHeight = playerBottomPos + (jumpHeight + parseInt(
                        playerPosY.substring(
                            0,
                            (playerPosY.length - 2)
                        )
                    )) + playerHeight;
...
function findObjectOnPos(x1, x2, y1, y2) {
    ...
    for (var i = 0; i < allObjects.length; i++) {
        ...
        if ((x1 <= offsetLeft && x2 >= offsetLeft) && (y1 <= offsetBottom && y2 >= offsetBottom)) {
          ...
          if (typeof(document.getElementById("foundObject")) != 'undefined' && document.getElementById("foundObject") != null) {
